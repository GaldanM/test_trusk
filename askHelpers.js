...
async function askEmployees() {
	...
	async function askEmployeeNames(max, current = 0,  accumulator = []) {
		if (current >= max) {
			return accumulator;
		}
		...
		return askEmployeeNames(max, current + 1, accumulator);
	}


	const registeredEmployeesNumber = await redisHelpers.getKey('nbEmployees');
	const nbEmployees = Number.parseInt(registeredEmployeesNumber) || await askEmployeesNumber();
	...
	return askEmployeeNames(nbEmployees, registeredEmployees.length, registeredEmployees);
}
async function askTrucks() {
	...
	async function askTrucksSpecs(max, current = 0, accumulator = []) {
		if (current >= max) {
			return accumulator;
		}
		...
		return askTrucksSpecs(max, current + 1, accumulator);
	}


	const registeredTrucksNumber = await redisHelpers.getKey('nbTrucks');
	const nbTrucks = Number.parseInt(registeredTrucksNumber) || await askTrucksNumber();
	...
	if (registeredTrucks.length >= nbTrucks) {
		return registeredTrucks;
	}

	return askTrucksSpecs(nbTrucks, registeredTrucks.length, registeredTrucks);
}
